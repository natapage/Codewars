...
function formatDuration(seconds) {
  ...
  const minute = Math.floor(
    (seconds - year * 31536000 - day * 86400 - hour * 3600) / 60
  );
  const second = Math.floor(
    seconds - year * 31536000 - day * 86400 - hour * 3600 - minute * 60
  );

  const obj = { year, day, hour, minute, second };
  const parts = Object.entries(obj).map(toString).filter(Boolean);
  ...
  return parts.reduce(
    (acc, item, index) =>
      acc + (index === parts.length - 1 ? " and " : ", ") + item
  );
}
